...
def main() -> int:
    ...
    if not args.offline:
        ...
        net_diag = {"generated_utc": now_utc(), "apihost": APIHOST,
        ...
        dump_json({"status_code": psc, "request": my_book, "response": presp,
                   "api_key_sha256": api_key_sha}, ART / "mybook_post.json")
        log(f"POST /books mybook: status={psc} id={my_id}")

        # 5) Add 100 faker books
        if Faker is None:
            raise RuntimeError("faker not installed")
        fake = Faker()
    ...
    def sha_file_if_exists(p: Path) -> str:
        return sha256_text(p.read_text(encoding="utf-8")) if p.exists() else ""
