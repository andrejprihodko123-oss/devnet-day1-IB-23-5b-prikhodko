...
def main():
    ...
    webex_room    = read(ART/"webex"/"room_create.json")
    webex_ok      = th8 in webex_room if webex_room else False
    pt_check_ok   = (ART/"pt"/"external_access_check.json").exists()
    pt_devices_ok = (ART/"pt"/"network_devices.json").exists()
    ...
    summary = {
        ...
        "pt": {
            "ok": pt_check_ok and pt_devices_ok,
            "empty_ticket_seen": contains(ART/"pt"/"external_access_check.json", "empty ticket"),
            "evidence_sha": {
        ...
        "validation_passed": bool(yang_ok and pt_check_ok),
