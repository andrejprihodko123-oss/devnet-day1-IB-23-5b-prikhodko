...
def test_day3_lab_artifacts_and_rules():
    ...
    for fn in [
        "books_before.json", "books_sorted_isbn.json", "mybook_post.json",
        "books_by_me.json", "add100_report.json", "postman_collection.json",
        "postman_environment.json", "curl_get_books.txt",
        "curl_get_books_isbn.txt", "curl_get_books_sorted.txt", "summary.json"
    ]:
        assert (DAY3 / fn).exists(), f"missing {fn}"

    summary = jload(DAY3 / "summary.json")
    jsonschema.validate(instance=summary, schema=jload(SCHEMA))
